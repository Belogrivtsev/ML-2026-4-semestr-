...
# ------------------------------------------------------------
# Загрузка моделей (логика из рабочего файла)
# ------------------------------------------------------------
@st.cache_resource
def load_models():
    loaded = {}
...
# ------------------------------------------------------------
# Загрузка данных (логика из рабочего файла)
# ------------------------------------------------------------
@st.cache_data
def load_data():
    base_path = os.path.dirname(__file__) if '__file__' in locals() else "."
...
# ------------------------------------------------------------
# Боковое меню (навигация) – названия из нерабочего файла
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# СТРАНИЦА 2: О датасете (полное оформление)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
    ...
    if df is None:
        ...
        # Создаём 4 вкладки (добавлена PCA)
        tab1, tab2, tab3, tab4 = st.tabs(["Распределение", "Boxplot", "Корреляция", "PCA"])
        
        # --- Вкладка 1: Распределение целевой переменной ---
        with tab1:
        ...
        # --- Вкладка 2: Boxplot (time_left и ct_health) ---
        with tab2:
        ...
        # --- Вкладка 3: Полная корреляционная матрица ---
        with tab3:
        ...
        # --- Вкладка 4: PCA проекция на 2 главные компоненты ---
        with tab4:
...
# ------------------------------------------------------------
    ...
    if model is None:
        ...
        # ----------------------------------------------------
        # РЕЖИМ 1: Ручной ввод (существующий код)
        # ----------------------------------------------------
                ...
                try:
                    ...
                    if df is None:
                        ...
                        if missing_cols:
                            ...
                            if extra_cols:
                                st.warning(f"⚠️ Найдены лишние колонки (будут проигнорированы): {extra_cols}")
                            
                            # Приводим порядок колонок и делаем предсказание
                            X_pred = df_input[expected_cols].copy()
                            
                            # Опционально: проверить на пропуски
